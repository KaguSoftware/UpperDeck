...
export function CartDrawer({
  ...
  return (
    ...
    >
      <div className="w-full bg-bg border-t-4 border-green animate-[slideUp_0.25s_cubic-bezier(0.2,0.8,0.2,1)] max-h-[85%] flex flex-col">
        {/* header */}
        <div className="flex items-center justify-between px-4.5 py-3 border-b-2 border-green shrink-0">
          <span className="font-bowlby text-[20px] text-green uppercase tracking-[-0.5px]">{totalLabel}</span>
          <button
            type="button"
            onClick={onClose}
        ...
        {items.length > 0 && (
          <div className="shrink-0 border-t-2 border-green">
            <div className="flex items-center justify-between px-4.5 py-2.5">
              <span className="font-extrabold text-[9px] tracking-[0.28em] text-green uppercase">{subtotalLabel}</span>
            ...
            <div className="px-4.5 pb-4">
              ...
              {process.env.NODE_ENV === "development" && onSimulateFailureChange && (
